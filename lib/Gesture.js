  ...
  componentWillMount() {
    ...
    this.panResponder = PanResponder.create({
      ...
      onMoveShouldSetPanResponderCapture: (e, { dx, dy, moveX, moveY }) => {
        ...
        if (global.activeScreen === 'projects') {
          if ((this.touchXStart < width - 60)) {
            return false
          }
        } else if (global.activeScreen === 'memeSelect') {
          if ((this.touchXStart > 60)) {
            return false
          }
        } else {
        ...
        return (Math.abs(dy) > 10 || Math.abs(dx) > 10) && onSwipeShouldAllow()
      },
